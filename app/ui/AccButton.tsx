...
const AnimatedButton = ({ onClick, status }: { onClick: () => void, status: "authenticated" | "loading" | "unauthenticated" }) => {
    let buttonContent;
    let buttonClass = "py-1 px-1 border-2 rounded-full cursor-pointer w-[60px] h-[60px] flex items-center justify-center";

    switch (status) {
        case "authenticated":
            buttonContent = <Image src={mikan.src} alt="MikanDev" width={50} height={50} />;
        ...
        case "unauthenticated":
            ...
            break;
        case "loading":
            buttonContent = <PiSpinnerBold className="animate-spin text-white" size={40} />;
            ...
            <AnimatePresence>
                {open && status === "authenticated" && (
                    ...
                    >
                        ...
                        <Center>
                            <Heading size="md" className="text-primary mb-5">MikanDev Account</Heading>
                        </Center>
                        <Center>
                            <Image src={session.user.image} alt="MikanDev" width={80} height={80} className="rounded-full" />
                        </Center>
                        <Center className="mt-5">
                        <Heading size="sm" className="text-primary">{session.user.name}</Heading>
                        </Center>
                        <Center>
